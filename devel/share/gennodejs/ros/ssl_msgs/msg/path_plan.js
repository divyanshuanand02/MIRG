...
class path_plan {
  constructor(initObj={}) {
    if (initObj === null) {
      ...
      if (initObj.hasOwnProperty('static_obstacle')) {
        this.static_obstacle = initObj.static_obstacle
      }
      else {
        this.static_obstacle = [];
      }
  ...
  static serialize(obj, buffer, bufferOffset) {
    ...
    // Serialize message field [static_obstacle]
    // Serialize the length for message field [static_obstacle]
    bufferOffset = _serializer.uint32(obj.static_obstacle.length, buffer, bufferOffset);
    obj.static_obstacle.forEach((val) => {
  ...
  static deserialize(buffer, bufferOffset=[0]) {
    ...
    // Deserialize message field [static_obstacle]
    // Deserialize array length for message field [static_obstacle]
    len = _deserializer.uint32(buffer, bufferOffset);
    data.static_obstacle = new Array(len);
    for (let i = 0; i < len; ++i) {
      data.static_obstacle[i] = StaticObstacle.deserialize(buffer, bufferOffset)
    }
  ...
  static getMessageSize(object) {
    ...
    length += 24 * object.static_obstacle.length;
    return length + 112;
  }
  ...
  static Resolve(msg) {
    ...
    const resolved = new path_plan(null);
    ...
    if (msg.static_obstacle !== undefined) {
      resolved.static_obstacle = new Array(msg.static_obstacle.length);
      for (let i = 0; i < resolved.static_obstacle.length; ++i) {
        resolved.static_obstacle[i] = StaticObstacle.Resolve(msg.static_obstacle[i]);
      }
    }
    else {
      resolved.static_obstacle = []
    }
